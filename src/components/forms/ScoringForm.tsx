...
function parseStoredScoreType(stored: string): { scoreType: string; patType?: string } {
  if (stored.includes(' + PAT')) {
    const plusIdx = stored.lastIndexOf(' + PAT');
    const base = stored.slice(0, plusIdx).trim();
    const pat = stored.slice(plusIdx + 3).trim(); // skip ' + '
    return { scoreType: base, patType: pat };
  }
  if (stored.endsWith(' (no PAT)')) {
    return { scoreType: stored.replace(' (no PAT)', '').trim() };
  }
  return { scoreType: stored };
}

export function ScoringForm({
  gameId, homeName, awayName, homeScore, awayScore, currentQuarter, currentClock, initialData, onSave, onClose,
}: ScoringFormProps) {
  const parsed = initialData?.score_type ? parseStoredScoreType(initialData.score_type) : undefined;
